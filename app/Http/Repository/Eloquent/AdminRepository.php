...
class AdminRepository extends AbstractRepository
{
    ...
    {
        return $this->model->latest()->where('is_teacher', 0)->paginate(10);
    }
    ...
    public function AdminGetMore($request)
    {
        try{
            if(isset($request->id) && $request->id > 0){
                $admins = $this->model->latest()->where('is_teacher', 0)->skip($request->id)->limit(10)->get();
            }else{
                $admins = $this->model->latest()->where('is_teacher', 0)->skip(10)->limit(10)->get();
            }
            $alladmins = [];
    ...
    public function AdminSearch($request)
    {
        try{
            ...
            if($query != ''){
                ...
                $admins = $this->model->latest()->where('is_teacher', 0)->paginate(10);
            }
            $alladmins = [];
            if($admins && count($admins) > 0){
                foreach($admins as $admin ){
                    if($admin->is_teacher == 0){
                        $admin->photo = asset($admin->photo);
                        $admin->role_id = optional($admin->role)->name;
                        $admin->urlRouteActivate = route('admins/activate', $admin->id);
                        $admin->urlRouteDelete = route('admins/delete', $admin->id);
