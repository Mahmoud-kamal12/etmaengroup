                        ...
                        <div class="card-body">
                            ...
                            <div class="btn-popup pull-right">
                                <button class="btn btn-primary btn-xs mt-1" type="button">
                                    <a href="{{route('admin/courses/create')}}" style="text-decoration:none;color:white;">
                                        اضافه
                                    </a>
                                </button>
                            </div>
                            ...
                            @isset($courses)
                                ...
                                <div class="card-body vendor-table">
                                                ...
                                                <tr>
                                                    <?php
                                                        ...
                                                        if($course->departement == 1){
                                                            $departement = 'استشاري';
                                                        }elseif($course->departement == 2){
                                                            $departement = 'لغات';
                                                        }elseif($course->departement == 3){
                                                            $departement = 'كورسات';
                                                        }
                                                    ?>
                                                    <td>{{$course->name}}</td>
                                                    ...
                                                    <div class="ml-2 d-flex">
                                                            <img src="{{asset($course->image)}}" class="img-fluid img-50 rounded-circle blur-up lazyloaded" alt=""></td>
                                                        </div>
                                                    <td>
                                                        <!-- edit row -->
                                                            <button class="btn btn-warning btn-xs" type="button">
                                                                <a href="{{route('admin/courses/edit', $course->id)}}" style="text-decoration:none;color:white;">
                                                        ...
                                                        <!-- delete row -->
                                                            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-original-title="test" data-bs-target="#myModaldelete-{{$course->id}}">
                                                                <i class="fas fa-trash-alt"></i>
                                                            </button>
                                                            <div class="modal fade" id="myModaldelete-{{$course->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                                <div class="modal-dialog" role="document">
                                                                    <div class="modal-content">
                                                                        ...
                                                                        <div class="modal-body">
                                                                            <form role="form" action="{{route('admin/courses/delete',$course->id)}}" class="" >
                                                                                <input name="_method" type="hidden">
                                                                                {{ csrf_field() }}
                                                                                <p>Are You Sure To Delete This Course ({{$course->name}}) ?</p>
                                    ...
                                    <div style="margin-top: 20px; font-weight: 600; font-size: 16px;">
                                        Showing 1 to <span id="showItemsCount"></span> of <span>{{App\Models\Courses::count()}}</span> entries
                                    </div>
...
@section('script')
<script>
    let dataLen = @json($courses);
    console.log(dataLen)
    let showItems = document.getElementById("showItemsCount");
    ...
    $(document).ready(function() {
        $(document).on('click', '#load_more_button', function() {
            ...
            function load_data(id = "", _token) {
                $.ajax({
                    url: "{{ route('admin/courses/getMore') }}",
                    method: "POST",
                    data: {
                        id: id,
                        _token: _token,
                    ...
                    success: function(data) {
                        if (data.length > 0) {
                                ...
                                records +=
                                            `
                                                <tr>
                                                    ...
                                                    <td>${data[i].course_duration}</td>
                                                    <td>${data[i].departement}</td>
                                                    <td><img src="{asset(${data[i].image})}" /></td>
                                                    <td>
                                                        <!-- edit row -->
                                                            <button class="btn btn-warning btn-xs" type="button">
                                                                    ...
                                                                    <div class="modal-content">
                                                                        ...
                                                                        <div class="modal-body">
                                                                            <form role="form" action="${data[i].urlRouteDelete}" class="" >
                                                                                <input name="_method" type="hidden">
                                                                                {{ csrf_field() }}
                                                                                <p>Are You Sure To Delete This Course (${data[i].name}) ?</p>
                                                                                <div class="modal-footer">
                                                                                    <button class="btn btn-primary" type="submit">Sure</button>
                                                                                    <button class="btn btn-secondary" type="button" data-bs-dismiss="modal">Cancel</button>
                                                                                </div>
                            ...
                            document.getElementById("coursesRecords").innerHTML += records
                            length += data.length
    ...
    $(document).on('keyup', '#data_search', function() {
        ...
        $.ajax({
            url: "{{ route('admin/courses/search') }}",
            method: "POST",
            data: {
                query: query,
                _token: _token
                ...
                if (data.length > 0) {
                    ...
                    document.getElementById("coursesRecords").style.display = null
                    document.getElementById("coursesRecords").innerHTML = records
                    $('#load_more_button').remove();
                    ...
                    length = data.length
                    showItems.innerHTML = Number(length)
                    document.getElementById("coursesRecords").style.display = 'none'
                    let btnNoData = `<button type="button" name="load_more_button" class="w-auto btn btn-primary form-control px-5" id="load_more_button_remove">No Data</button>`
                    document.getElementById("load_more").innerHTML = btnNoData
                }
            }
